...
<html>
    <head>
        ...
        <?php 
            ...
            //$costume_sql = "SELECT DISTINCT SUBSTRING_INDEX(name,'-',LENGTH(name)-LENGTH(REPLACE(name,'-',''))) as costume_name FROM costume_design WHERE customized_design_approval = 'approve' AND final_price IS NOT NULL";
            $costume_sql = "SELECT DISTINCT SUBSTRING_INDEX(name,'-',LENGTH(name)-LENGTH(REPLACE(name,'-',''))) as costume_name FROM costume_design WHERE customized_design_approval = 'approve' AND final_price > 0 AND merchandiser_id IS NOT NULL;";
            $costumeNameCount = 0;
            $costumeNameList = "";
